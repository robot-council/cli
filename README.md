...
**`--keep-warm-for=<minutes>` is the ceiling.** Once the session has been idle that long, keep-alives stop, so a seat abandoned overnight is not woken every hour until morning. Without it they go on for as long as the bridge runs. Either option given a value that is not a whole number of minutes, or `--keep-warm` given to any harness but Claude Code, is reported on stderr and the bridge runs without keep-alives.

**What counts as activity, and what the bridge cannot see.** The bridge sees no turns ([#211](https://github.com/robot-council/cli/issues/211) records why), so it reads three things that stand in for them: a tool call arriving, a notice it sent about fleet events, and a turn end the stop hook recorded. `robot-council pending` records the time it drains the sink, in a file beside the sink, and `--peek` does not. So:

- **It needs the stop hook below, and Claude Code started with channels**, as [waking a seat](https://github.com/robot-council/cli/wiki/Starting-a-seat-so-it-can-be-woken) describes. Without the hook, a session working only in Bash and editing files looks idle, and gets a keep-alive it will fold into the turn it is running. Without channels, the notice is dropped and nothing is kept warm.
- **A keep-alive's own turn end is not activity**, or the ceiling could never arrive: the first turn end within two minutes of a keep-alive is taken as that keep-alive's. A tool call in between means the agent did something, and the turn end after it counts.
...
**What the woken turn does has not been measured yet.** The server instructions tell the agent that a notice carrying `keep_warm` has nothing behind it and to end the turn without tool calls; whether a live session does, and that the cache stays warm, are open on [#279](https://github.com/robot-council/cli/issues/279).
