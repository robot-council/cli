...
3. **Use the REST equivalents for issue and pull-request work.** These keep working when `gh issue` does not:
   ...
   | Assign (replace) | `gh api -X POST repos/{o}/{r}/issues/{n} -f 'assignees[]=handle'` |
   | Assign (add) | `gh api -X POST repos/{o}/{r}/issues/{n}/assignees -f 'assignees[]=handle'` |
   | Open a PR | `gh api -X POST repos/{o}/{r}/pulls --input -` with a JSON body (`title`, `head`, `base`, `body`, `draft`) |
   | Merge a PR | `gh api -X PUT repos/{o}/{r}/pulls/{n}/merge -f merge_method=<merge\|squash\|rebase>` |
